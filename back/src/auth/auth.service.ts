...
export class AuthService {
	...
	private async generateAccessToken(memberId: number) {
		return this.jwtService.signAsync({ sub: memberId }, { secret: process.env.JWT_SECRET, expiresIn: "15m" });
	}

	private async generateRefreshToken(memberId: number, userAgent: string | undefined, ipAddress: string | undefined) {
		const token = await this.jwtService.signAsync({ sub: memberId }, { secret: process.env.JWT_REFRESH_SECRET, expiresIn: "7d" });
